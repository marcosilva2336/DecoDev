...
function filterProjectsByCategory(category) {
  ...
  document.querySelectorAll('.cases__link').forEach(link => {
    link.classList.remove('active');
  });

  // Adicionar a classe 'active' ao link da categoria selecionada
  document.querySelector(`.cases__link[data-category="${category}"]`).classList.add('active');

  // Filtragem dos projetos
  if (category === 'all') {
    // Mostrar todos os projetos para a categoria 'all'
    document.querySelectorAll('.cases__item').forEach(item => {
      item.style.display = 'block';
    });
  } else {
    // Esconder todos os projetos e mostrar apenas os da categoria selecionada
    document.querySelectorAll('.cases__item').forEach(item => {
...
// Evento de clique para filtrar projetos
document.querySelectorAll('.cases__link').forEach(link => {
...
// Filtrar por 'all' no carregamento da página
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Seleciona todos os links do cabeçalho e do rodapé que levam a uma âncora na página
  const links = document.querySelectorAll('.header__item[href^="#"]');
  ...
  const smoothScrollTo = (element) => {
    ...
    const duration = 5000; // Aumenta a duração para 3 segundos
    let startTime = null;
